...
- Building from source now needs Go 1.26 or later (was 1.25). golang.org/x/sync v0.23.0, which this update takes, declares `go 1.26.0`, and Go 1.26 and 1.27 are the two releases the Go team still supports. Prebuilt binaries are unaffected.
...
answers "no published release" without saying whether the tags it checked had
no release behind them, were drafts, or were pre-releases, and a release that
carries no assets at all says so instead of listing none.
