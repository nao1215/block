...
func TestResolvePicksNewestPublishedRelease(t *testing.T) {
	...
	url, err := Artifact(res, src(), platform.Platform{OS: "linux", Arch: "amd64"})
	if err != nil || url != "https://dl/v1.7.4/t_1.7.4_linux_amd64.tar.gz" {
		t.Errorf("Artifact() = %s, %v", url, err)
	}
	_, err = Artifact(res, src(), platform.Platform{OS: "darwin", Arch: "arm64"})
	...
	_, err = Artifact(res, limited, platform.Platform{OS: "darwin", Arch: "arm64"})
	var unsupported *recipe.UnsupportedPlatformError
	if !errors.As(err, &unsupported) {
		t.Errorf("Artifact(unsupported) error = %v", err)
	}
}
