...
var (
	...
	NoPublishedRelease = register(2002,
		...
		"For a tag with no release or a draft, wait for the upstream to publish. For a pre-release, either wait for a final release or, when the line only ever ships pre-releases, pin an older version that has a published release. An upstream that publishes nothing but pre-releases cannot be pinned by version.",
		"v0.1.0")
